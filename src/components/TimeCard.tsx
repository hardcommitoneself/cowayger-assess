...
const TimeCard: Component<ITimeCard> = (props) => {
    ...
    const [state, { handleOrder }] = useTimedOrderContext()

    return <div class={`flex justify-center items-center py-4 border rounded-xl cursor-pointer relative ${state.currentSelectedOrder === index ? 'bg-green-500' : ''}`} onClick={() => handleOrder(index)}>
